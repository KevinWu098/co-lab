...
export function Chat({ expanded = false, onToggleExpand }: ChatProps) {
  ...
  return (
    <div className={`flex h-full ${expanded ? "flex-1" : ""}`}>
      {expanded && onToggleExpand && (
        <button
          aria-label="Collapse chat"
          className="text-muted-foreground group flex w-6 shrink-0 cursor-pointer items-center justify-center border-y border-l opacity-0 transition-opacity duration-150 hover:opacity-100"
          onClick={onToggleExpand}
          type="button"
        >
          <Minimize2Icon className="size-3 opacity-0 transition-opacity duration-150 group-hover:opacity-100" />
        </button>
      )}

      <div className={`bg-background flex h-full flex-col border ${expanded ? "flex-1" : "w-xs"}`}>
